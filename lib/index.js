  ...
  EPub = class EPub {
    ...
    generateTempFile() {
      ...
      _.each(this.options.content, function(content) {
        ...
        data += content.title && self.options.appendChapterTitles ? `<h1 class="h1">${entities.encodeXML(content.title)}</h1>` : "";
    ...
    makeCover() {
      var coverDefer, destPath, userAgent, writeStream, coverpath;
      ...
      if (this.options.cover) {
        ...
        coverpath = path.resolve(this.uuid, "./marketing/external/Cover." + this.options._coverExtension);
        writeStream = null;
        ...
        writeStream.on("end", async function() {
          ...
          // 新建放封面的目录
          await mkdir(path.resolve(self.uuid, "./marketing"))
          await mkdir(path.resolve(self.uuid, "./marketing/external"))
          await cpFile(destPath, coverpath);
          // const cmd = `cp ${destPath} ${coverpath}`;
          // execSync(cmd);
    ...
    genEpub() {
      ...
      // archive.file(cwd + "OEBPS/cover.jpeg",{name: 'marketing/external/Cover.jpeg'});
      // archive.file(cwd + "OEBPS/cover.jpeg", { name: 'Cover.jpeg'});
      archive.directory(cwd + "/META-INF", "META-INF");
      archive.directory(cwd + "/OEBPS", "OEBPS");
      // archive.directory(cwd + "/OEBPS", "marketing/external");
      archive.directory(cwd + "/marketing/external", "marketing/external");
      archive.pipe(output);
